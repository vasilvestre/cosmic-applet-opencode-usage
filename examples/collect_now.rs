...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_path = std::path::PathBuf::from(&home).join(".local/share/cosmic-applet-opencode-usage/usage.db");
    
    println!("Database: {}", db_path.display());

    // Create/open database
    let db = Arc::new(DatabaseManager::new_with_path(&db_path)?);
    let repo = cosmic_applet_opencode_usage::core::database::repository::UsageRepository::new(Arc::clone(&db));
    ...
    println!("  Reasoning tokens:  {:>12}", metrics.total_reasoning_tokens);
    println!("  Cache write:       {:>12}", metrics.total_cache_write_tokens);
    println!("  Cache read:        {:>12}", metrics.total_cache_read_tokens);
