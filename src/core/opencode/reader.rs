...
mod tests {
    ...
    fn test_reader_cache_preserved_across_mode_switches() {
        ...
        assert_eq!(today_metrics.total_input_tokens, 350, "Should only count today's files");
        assert_eq!(today_metrics.interaction_count, 2);
        
        // Check cache after get_usage_today()
        eprintln!("Cache after get_usage_today: exists={}, files={}", 
        ...
        assert_eq!(cache_size_after_switch, 3, 
    ...
    fn test_reader_filter_month_only() {
        ...
        filetime::set_file_mtime(&file1_path, filetime::FileTime::from_system_time(last_month))
            .expect("Failed to set file time");
        ...
        let metrics = reader.get_usage_month().expect("Should read this month's data");
        
        // Should only include file2 and file3 (within this month)
        assert_eq!(metrics.total_input_tokens, 350, "Should only count this month's files");
    ...
    fn test_month_start_calculation() {
        ...
        let month_start_duration = month_start.duration_since(UNIX_EPOCH)
            .expect("Should get duration");
        let month_start_secs = month_start_duration.as_secs() as i64;
        let month_start_dt = chrono::Local.timestamp_opt(month_start_secs, 0)
            .single()
        ...
        assert_eq!(month_start_dt.month(), now.month(), "Should be current month");
    ...
    fn test_reader_filter_calendar_month() {
        ...
        let this_month_time = Local.with_ymd_and_hms(now.year(), now.month(), 5, 12, 0, 0)
            .single()
        ...
        }.single().expect("Should create valid date");
        filetime::set_file_mtime(
            &last_file,
        ...
        let two_months_ago = if now.month() <= 2 {
            Local.with_ymd_and_hms(now.year() - 1, (12 + now.month() as i32 - 2) as u32, 1, 12, 0, 0)
        } else {
        ...
        assert_eq!(metrics.total_input_tokens, 100, "Should only count current calendar month files");
